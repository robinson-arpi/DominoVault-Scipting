...
REQUEST_TIMEOUT = 15  # Tiempo de espera en segundos
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if response.status_code == 504:
                print("Error 504: Gateway Timeout. Retrying...")
                ...
                print("Please check if the document already exists or if there are conflicting changes.")
...
def search_folder_by_name(folder_name):
    try:
        ...
        print(f"Error searching the folder: {e}")
        return None
...
def upload_document_to_alfresco(parent_id, file_path):
    try:
        ...
        if response is not None and response.status_code == 201:
            node = response.json()["entry"]
            print(f"Document uploaded: {node['name']} (ID: {node['id']})")
